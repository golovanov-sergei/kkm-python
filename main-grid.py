...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        mainframe.pack(fil=tk.BOTH)
        # toolbar = ttk.Frame(self, relief=tk.RAISED,borderwidth=1)
        # toolbar.pack(side=tk.TOP, fill=tk.X)

        # self.add_img = tk.PhotoImage(file="add.gif")
        btn_open_dialog = tk.Button(mainframe, text='Штрихкод (F7)', command=self.open_dialog, bg='#d7d8e0', bd=1,
                                    compound=tk.TOP, width=16)
        btn_open_dialog.grid(row=0, column=0, sticky=tk.W)

        btn_addGood = tk.Button(mainframe, text='Добавить товар', command=self.open_addGood, bg='#d7d8e0', bd=1,
                                compound=tk.TOP, width=16)
        btn_addGood.grid(row=0, column=1)

        btn_delGood = tk.Button(mainframe, text='Удалить', command=self.DeleteGood, bg='#d7d8e0', bd=1,
                                compound=tk.TOP, width=16)
        btn_delGood.grid(row=0, column=2)

        btn_Seller = tk.Button(mainframe, text='Продавец', command=self.selectSeller, bg='#d7d8e0', bd=1,
                               compound=tk.TOP, width=16)
        btn_Seller.grid(row=0, column=3)

        btn_CloseCheck = tk.Button(mainframe, text='Оплата', command=self.closeCheck, bg='#d7d8e0', bd=1,
                                   compound=tk.TOP, width=16)
        btn_CloseCheck.grid(row=0, column=4, sticky=tk.E)

        # frmMain = ttk.Frame(self,borderwidth=1)
        # frmMain.pack(fill=tk.X)

        self.tree = ttk.Treeview(mainframe, columns=('No', 'barcode', 'description', 'price'),
        ...
        # frmTotal = tk.Frame(self,borderwidth=1,relief=tk.SUNKEN)
        # frmTotal.pack(fill=tk.BOTH)
        label_seller = tk.Label(mainframe, text='Продавец:')
        label_total = tk.Label(mainframe, text='Итого:')
        # label_total.pack(fill=tk.X,anchor=tk.E)
        label_seller.grid(row=2, column=0, columnspan=2)
        label_total.grid(row=2, column=2, columnspan=2)
